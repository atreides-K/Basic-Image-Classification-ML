...
def run_plainnet_experiment(n_value: int, depth: int):
    """Runs a training experiment for a PlainNet with a given n."""
    ...
    test_transform = transforms.Compose([
       ...
       # Neo train transform
    #  reference from https://github.com/kuangliu/pytorch-cifar 
    ...
    model = PlainNet(n=config["n_value"]) # Pass the calculated n_value
    ...
    if config["optimizer"] == "SGD":
        optimizer = optim.SGD(model.parameters(),
         ...
         raise ValueError("Unsupported optimizer for plainsNet experiments") # Typically use SGD
    ...
    # --- Final Evaluation ---
    print(f"\nFinal Evaluation for plainNet-{depth}:")
...
if __name__ == '__main__':
    ...
    # run_plainnet_experiment(n_value=3, depth=20)
    # run_resnet_experiment(n_value=9, depth=56)
    run_plainnet_experiment(n_value=18, depth=110)
